...
The project uses SiliconFlow's DeepSeek API:

```bash
# SiliconFlow DeepSeek (default)
...
## Using WikiEval Dataset

The project also supports using the [WikiEval dataset](https://huggingface.co/datasets/explodinggradients/wikieval) - a high-quality Wikipedia-based RAG evaluation dataset.

### Download WikiEval Dataset
...
### Run Evaluation with WikiEval

```bash
# Run evaluation using WikiEval dataset
uv run python evals_wikieval.py
```

This will:
1. Load WikiEval dataset from local or Hugging Face
2. Use samples as the RAG knowledge base
3. Evaluate RAG performance on test questions
4. Generate detailed reports with pass/fail metrics
